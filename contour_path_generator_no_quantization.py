#!/usr/bin/env python
# -*- coding: utf-8 -*-
# ABLATION STUDY VERSION: w/o altitude quantization
...
def process_single_task_area(task_area_identifier, input_dir, base_output_dir, args):
    ...
    print(f"\n{'='*20} Processing Task Area: {task_area_identifier} (NO ALTITUDE QUANTIZATION) {'='*20}")
    ...
    print(f"  ABLATION: Using actual terrain elevation (no quantization)")
    ...
    def generate_path_primitive_local(seed_x, seed_y, min_points=3):
        ...
        for pt_2d in path_pts_2d_list:
            ...
            # ABLATION: Use actual terrain elevation (no quantization)
            if np.isnan(terrain_z_at_pt):
                final_pt_z = contour_elev_val  # Fallback to contour if terrain is NaN
            else:
                final_pt_z = terrain_z_at_pt  # Use actual terrain elevation directly
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Generate contour-based path primitives for diverse task areas (ABLATION: w/o altitude quantization)')
    ...
    parser.add_argument('--output-dir', default='path_primitives_diverse_no_quantization', help='Output directory for path primitives (ABLATION: no altitude quantization)')
    ...
    print(f"Found {len(task_area_identifiers)} diverse task areas to process (ABLATION: w/o altitude quantization):")
    ...
    print(f"\nAll diverse task areas processed (ABLATION: w/o altitude quantization) in {total_time:.2f}s")
