#!/usr/bin/env python
# -*- coding: utf-8 -*-
# ABLATION STUDY VERSION: w/o ATSP (greedy algorithm)
...
parser = argparse.ArgumentParser(description="ABLATION: Greedy Connection Algorithm (w/o ATSP) for Diverse Task Areas")
...
parser.add_argument('--output_dir', type=str, default='capg_tco_paths_diverse_greedy', help='Directory to save final paths and visualizations (ABLATION: greedy).')
...
def solve_greedy_connection(tsp_nodes, energy_model, dem_data, x_m, y_m):
    """
    ABLATION: Greedy algorithm instead of ATSP optimization
    Selects the best direction for each primitive and connects them greedily
    """
    print("  ABLATION: Using greedy connection algorithm (w/o ATSP)...")
    
    # Group nodes by physical primitive
    ...
    # Select best direction for each primitive (lowest traversal energy)
    selected_primitive_nodes = []
    for phys_id, node_indices in physical_prim_to_tsp_nodes.items():
        if len(node_indices) == 2:
            # Compare forward and backward directions
            forward_node = tsp_nodes[node_indices[0]]
            backward_node = tsp_nodes[node_indices[1]]
            
            if forward_node['traversal_energy'] <= backward_node['traversal_energy']:
                selected_primitive_nodes.append(forward_node['id'])
                print(f"    Primitive {phys_id}: selected forward direction (energy: {forward_node['traversal_energy']:.2f})")
            else:
                selected_primitive_nodes.append(backward_node['id'])
                print(f"    Primitive {phys_id}: selected backward direction (energy: {backward_node['traversal_energy']:.2f})")
        elif len(node_indices) == 1:
            selected_primitive_nodes.append(node_indices[0])
            print(f"    Primitive {phys_id}: only one direction available")

    print(f"  Selected {len(selected_primitive_nodes)} primitive nodes for greedy connection")
    
    # Build energy cost matrix only for selected nodes plus depot
    all_nodes_for_greedy = [0] + selected_primitive_nodes  # 0 is depot
    num_nodes = len(all_nodes_for_greedy)
    energy_matrix = np.full((num_nodes, num_nodes), np.inf)
    
    print("  Building energy cost matrix for greedy algorithm...")
    for i in range(num_nodes):
        tsp_node_i = tsp_nodes[all_nodes_for_greedy[i]]
        ...
        for j in range(num_nodes):
            ...
            tsp_node_j = tsp_nodes[all_nodes_for_greedy[j]]
    ...
    # Greedy nearest neighbor algorithm starting from depot
    print("  Solving with greedy nearest neighbor algorithm...")
    visited = set()
    current_node_idx = 0  # Start from depot (index 0 in our reduced matrix)
    route = [current_node_idx]
    ...
    while len(visited) < num_nodes:
        ...
        for next_idx in range(num_nodes):
            if next_idx not in visited and energy_matrix[current_node_idx, next_idx] < best_energy:
                best_energy = energy_matrix[current_node_idx, next_idx]
                best_next_idx = next_idx
        
        if best_next_idx is not None:
            route.append(best_next_idx)
            visited.add(best_next_idx)
            total_energy += best_energy
            print(f"    Step {len(route)-1}: {current_node_idx} -> {best_next_idx}, energy: {best_energy:.2f}")
            current_node_idx = best_next_idx
        else:
            print("  WARNING: Greedy algorithm couldn't find next node")
            break
    
    # Return to depot
    if current_node_idx != 0:
        return_energy = energy_matrix[current_node_idx, 0]
        route.append(0)
        total_energy += return_energy
        print(f"    Return to depot: {current_node_idx} -> 0, energy: {return_energy:.2f}")
    
    print(f"  Greedy solution found. Total energy: {total_energy:.2f}")
    
    # Convert back to original node indices
    ordered_original_indices = [all_nodes_for_greedy[idx] for idx in route]
    print(f"  Greedy route (original indices): {ordered_original_indices}")
    
    return ordered_original_indices
...
def process_task_area(task_area_id, base_task_areas_dir, base_primitives_dir, base_output_dir, energy_model_instance):
    ...
    print(f"Mode: ABLATION - Greedy Connection (w/o ATSP) - Diverse Areas")
    ...
    # Record greedy solving time
    greedy_start_time = time.time()
    
    # ABLATION: Use greedy connection instead of ATSP
    tsp_nodes = generate_tsp_nodes_from_primitives(physical_primitives, energy_model_instance, depot_3d_loc)
    if len(tsp_nodes) <= 1: 
        print(f"  Too few nodes ({len(tsp_nodes)}) generated for {task_area_id}. Skipping greedy connection.")
        return
        
    tsp_ordered_indices = solve_greedy_connection(tsp_nodes, energy_model_instance, dem_data, x_m, y_m)

    if tsp_ordered_indices:
        print(f"  Greedy node sequence (our indices): {tsp_ordered_indices}")
        ...
        print(f"  Physical primitives included in greedy solution: {len(visited_physical_primitive_ids)} out of {len(physical_primitives)}")
        ...
        print(f"  Could not generate greedy path for {task_area_id}.")
        return
    
    greedy_solving_time = time.time() - greedy_start_time
    ...
    runtime_info = {
        ...
        'greedy_solving_time': greedy_solving_time,
        'coverage_percent': coverage_percent,
        'ablation_type': 'greedy_connection'
    }
