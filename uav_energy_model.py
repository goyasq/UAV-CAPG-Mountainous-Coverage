...
class UAVEnergyModel:
    """
    UAV能量消耗模型 - DJI Matrice 300 RTK（典型平台参数化）
    依据 DJI 官方电池/续航数据反推悬停功率量级，并结合多旋翼功率分解（位移功+加速功）
    采用分段恒速、加减速瞬时的估计前提；参数可按载荷在"无载荷 / H20"两档切换。
    
    参数来源：悬停功率由 DJI 官方 TB60 电量与悬停时长反推（无载荷 55 min；H20 约 45 min），
    其余系数按重型四旋翼的经验比例设置，并保留下降功率不低于悬停功率 50% 的控制下限。
    
    证据与规格来源：TB60=274Wh×2；无载荷55min悬停；H20约45min悬停（详见官网）。
    
    通用性：模型形式与求解器平台无关；如更换载荷/平台，仅需替换 P_hover, k_h, P_asc, P_desc 
    即可在同一框架内重算能耗。
    """
    
    def __init__(self, 
                 use_h20_profile=False, # 是否使用H20载荷配置（默认无载荷）
                 m=6.3,                # 无人机总质量 (kg) - DJI M300 RTK + 双TB60电池
                 g=9.8,                # 重力加速度 (m/s^2)
                 P_hover=600.0,        # 悬停功率基线 (W) - 由2×TB60/55min反推（无载荷）
                 k_h=5.5,              # 水平速度系数 (W·s²/m²) - M210基础上略上调
                 P_asc=250.0,          # 上升功率系数 (W·s/m) - 重机上升更吃力
                 P_desc=130.0,         # 下降功率系数 (W·s/m) - 含50% P_hover安全下限
                 v_xy=5.0,             # 水平飞行速度 (m/s) - 典型巡航
                 v_z_asc=3.0,          # 上升速度 (m/s) - 典型上升
                 v_z_desc=2.0,         # 下降速度 (m/s) - 典型下降
        ...
        两套配置可选：
        - 无载荷（默认）：P_hover=600W, k_h=5.5, P_asc=250, P_desc=130
        - H20载荷：P_hover=730W, k_h=6.0, P_asc=280, P_desc=140
        
        参数来源：
        - P_hover: 由DJI官方TB60电池容量（274Wh×2）与悬停时长反推
        - 其余系数: 按重型四旋翼特性设置，保持功率分解模型的一致性
        """
        # 首先设置基本参数
        self.m = m
        self.g = g
        ...
        # 根据载荷配置选择功率参数
        if use_h20_profile:
            # H20载荷配置：由2×TB60/45min反推
            self.P_hover = 730.0
            self.k_h = 6.0
            self.P_asc = 280.0
            self.P_desc = 140.0
        else:
            # 无载荷配置（默认）：由2×TB60/55min反推
